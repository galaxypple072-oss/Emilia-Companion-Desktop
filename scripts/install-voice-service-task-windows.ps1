...
$taskName = "Emilia Voice Service"
$projectRoot = Split-Path -Parent $PSScriptRoot
$launcherPath = Join-Path $projectRoot "scripts\windows\start-voice-service.ps1"
$hiddenRunner = Join-Path $projectRoot "scripts\windows\run-hidden-powershell.vbs"
$userId = "$env:COMPUTERNAME\$env:USERNAME"
if (-not (Test-Path -LiteralPath $launcherPath)) { throw "Voice Service launcher not found: $launcherPath" }
...
Register-ScheduledTask -TaskName $taskName -Action $action -Principal $principal -Settings $settings -Description "Local voice API managed by Emilia Host Agent" -Force | Out-Null
