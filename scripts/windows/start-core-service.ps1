...
try {
  ...
  if (-not (Test-Path -LiteralPath $nodePath)) { $nodePath = "C:\Program Files\nodejs\node.exe" }
  $cliPath = Join-Path $projectRoot "apps\product-core\src\cli.ts"
  $env:EMILIA_ENV_PATH = Join-Path $env:LOCALAPPDATA "PersonalCompanion\.env"
  $env:PATH = (Join-Path $projectRoot "node") + ";" + $env:PATH
  ...
  while ($true) {
    ...
    $quotedCliPath = '"' + $cliPath + '"'
    $process = Start-Process -FilePath $nodePath -ArgumentList "--experimental-strip-types", $quotedCliPath, "run" -WorkingDirectory $projectRoot -RedirectStandardOutput $logPath -RedirectStandardError $errorLogPath -WindowStyle Hidden -PassThru
    $process.WaitForExit()
